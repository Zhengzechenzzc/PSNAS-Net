...
if __name__ == "__main__":
    ...
    # 定义模型
    model_ft = Container.container_v1_light(pretrained=False, num_classes=CLASSES)
    print(model_ft)
    ...
    accdf.to_csv('08container-acc.csv')
    lossdf = pd.DataFrame(lossnp)
    lossdf.to_csv('08container-loss.csv')
    # 训练结束
    time_elapsed = time.time() - since
    print()
    print('任务完成！')
