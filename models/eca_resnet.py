...
class ResNet(nn.Module):
    ...
    def __init__(self, params, block, layers, num_classes=1000, k_size=[3, 3, 3, 3]):
        self.inplanes = 64
        ...
        self.layer1 = self._make_layer(block, params['ch_1'], layers[0], int(k_size[0]))
        self.layer2 = self._make_layer(block, params['ch_2'], layers[1], int(k_size[1]), stride=2)
        self.layer3 = self._make_layer(block, params['ch_3'], layers[2], int(k_size[2]), stride=2)
        self.layer4 = self._make_layer(block, params['ch_4'], layers[3], int(k_size[3]), stride=2)
        self.avgpool = nn.AvgPool2d(7, stride=1)
        # self.fc = nn.Linear(512 * block.expansion, num_classes)
        self.fc = nn.Linear(params['ch_4'] * block.expansion, num_classes)
...
def eca_renetAny(params, k_size=[3, 3, 3, 3], num_classes=1_000, pretrained=False):

    model = ResNet(params, ECABasicBlock, [params['layer_1'], params['layer_2'], params['layer_3'], params['layer_4']],
                   num_classes=num_classes, k_size=k_size)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model

def eca_renetAny_Bottle(params, k_size=[3, 3, 3, 3], num_classes=1_000, pretrained=False):

    model = ResNet(params, ECABottleneck, [params['layer_1'], params['layer_2'], params['layer_3'], params['layer_4']],
                   num_classes=num_classes, k_size=k_size)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model


# def eca_resnet18(params, k_size=[3, 3, 3, 3], num_classes=1_000, pretrained=False):
...
#     """
#     model = ResNet(params, ECABasicBlock, [2, 2, 2, 2], num_classes=num_classes, k_size=k_size)
#     model.avgpool = nn.AdaptiveAvgPool2d(1)
#     return model
#
#
