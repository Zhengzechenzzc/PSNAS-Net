...
class ResNeXt(nn.Module):
    def __init__(self, params, block, num_classes=1000): #, layers
        super().__init__()
        #定义输入模块的维度
        self.in_channel=64
        ### stem layer
        ...
        self.layer1=self._make_layer(block, params['ch_1'], params['layer_1'])
        self.layer2=self._make_layer(block, params['ch_2'], params['layer_2'], stride=2)
        self.layer3=self._make_layer(block, params['ch_3'], params['layer_3'], stride=2)
        self.layer4=self._make_layer(block, params['ch_4'], params['layer_4'], stride=2)

        #classifier
        self.avgpool=nn.AdaptiveAvgPool2d(1)
        self.classifier=nn.Linear(params['ch_4']*block.expansion, num_classes)
...
def ResNextAny(params, num_classes=1000):
    model = ResNeXt(BottleNeck, params, num_classes=num_classes)
    return model
